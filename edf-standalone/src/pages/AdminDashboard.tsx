...
import { LogOut, Download, Search, ArrowUpDown, Check, UserCircle, Trash2, XCircle } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const handleExportExcel = () => {
    const headers = ["Nome", "Ano de Nascimento", "Ano de Escolaridade", "Turma",
      "Atividade 1", "Atividade 2", "Atividade 3", "Atividade 4", "Atividade 5", "Média"];

    const rows = filteredAndSortedData.map((reg, idx) => {
      const rowNum = idx + 2;
      return [
        reg.name, reg.birthYear, reg.schoolYear, reg.className,
        reg.activity1 ?? "", reg.activity2 ?? "", reg.activity3 ?? "",
        reg.activity4 ?? "", reg.activity5 ?? "",
  ...
  const cycleSort = () => {
    setSortBy((prev) => prev === "name" ? "birthYear" : prev === "birthYear" ? "className" : "name");
  };
    ...
    <tr className={`hover:bg-muted/30 transition-colors group ${isAbsent ? "opacity-60" : ""}`}>
      ...
      {(["activity1", "activity2", "activity3", "activity4", "activity5"] as const).map((act) => (
        <td key={act} className="px-2 py-4">
          <Input value={scores[act]} onChange={(e) => handleScoreChange(act, e.target.value)}
            disabled={isAbsent} className="w-16 h-9 text-center px-1 font-mono mx-auto disabled:opacity-40"
            placeholder="-" />
        </td>
      ))}
      ...
      <td className="px-4 py-4">
        <div className="flex items-center justify-center gap-1">
          {isDirty && (
            <Button size="sm" onClick={handleSave} className="h-8 w-8 p-0 shadow-md shadow-primary/20" title="Guardar notas">
          ...
          <button onClick={handleToggleAbsent} title={isAbsent ? "Remover falta" : "Marcar falta"}
            className={`h-8 w-8 flex items-center justify-center rounded-lg border transition-colors ${
              isAbsent ? "bg-destructive/15 border-destructive/30 text-destructive hover:bg-destructive/25"
                : "border-border text-muted-foreground hover:bg-destructive/10 hover:text-destructive hover:border-destructive/30"
            }`}>
            <XCircle className="w-4 h-4" />
          </button>
          <button onClick={handleDelete} title={confirmDelete ? "Clica novamente para confirmar" : "Apagar aluno"}
            className={`h-8 w-8 flex items-center justify-center rounded-lg border transition-colors ${
              confirmDelete ? "bg-destructive border-destructive text-white animate-pulse"
                : "border-border text-muted-foreground hover:bg-destructive/10 hover:text-destructive hover:border-destructive/30"
            }`}>
